...
const addToCart = async (req, res) => {
  ...
  if (isFoundItem) {
    ...
    if (userCart) {
      ...
      // let totalPrice;
      // function calcProductPrice(value, index, array) {
      ...
      // let productInCart = userCart.cartItems.map(item =>item.product);
      // console.log(productInCart);
    const userCartr = await Cart.findOne({ user: req.user.id }).cursor;

      const comments = []
      for ( let i = await userCart.next(); i != null; i = await cursor.next()) {
        const price = await Product.find({id:i.cartItems.product})
        const obj = {...i._doc,price}
        comments.push(obj)
      }
