...
const addOrder = async (req, res) => {
  try {
    ...
    const cart = await Cart.findOne({ user })
      .populate("cartItems.product", "_id name price imgURL category brand")
      .populate({path: "cartItems.product.category",model: "category", select: "name"});
    if (cart.cartItems.length === 0) {
      ...
      // create invoice
      createInvoice(order, `invoice/${req.user.id}-${Date.now()}.pdf`);
