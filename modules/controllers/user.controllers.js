...
const varificationCode = async (req, res) => {
  try {
    ...
    if (!confirmCode) {
      ...
      await User.findByIdAndUpdate({_id:confirmCode.userId}, {isVerified:true}, {new:true});
...
const resendConfirmationCode = async (req, res) => {
  ...
  if (!user) {
    ...
    const userCode = await ConfirmCode.findOne({ userId: user._id });
    if (!userCode) {
      const code = await generateCode(userData._id);
      const message = `<h2>${code}</h2>`;
      await sendEmail({
      ...
      userCode.code = hashCode;
      userCode.expireDate = expireDate;
...
const userData = async (req, res) => {
  try {
    const user = await User.findById(req.user.id).select("name email imgURL");
