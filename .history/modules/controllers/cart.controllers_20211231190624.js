  ...
  try {
    ...
    if (isFoundItem) {
      ...
      if (userCart) {
        ...
        if (item) {
          // Check if countInStock cover customer quantity
          let currentQuantity = item.quantity + req.body.cartItems.quantity;
          if (currentQuantity > isFoundItem.countInStock) {
            res.status(StatusCodes.BAD_REQUEST).json({
              message:
                "Maximum amount this product is " + isFoundItem.countInStock,
            });
          } else {
...
const changeProductQuantity = async (req, res) => {
  try {
    ...
    Cart.findOneAndUpdate(
      ...
      { new: true }
    ).populate({ path: "cartItems.product", model: "product" }).exec((error, cart) => {
      if(error) {
        res.status(
          StatusCodes.BAD_REQUEST).json({ message:"Error", error})
      }
    })
